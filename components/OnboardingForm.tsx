...
export const OnboardingForm: React.FC<OnboardingFormProps> = ({ onBack, initialStep = 1 }) => {
  ...
  const handleNextStep = async () => {
    ...
    try {
      if (currentStep === 1) {
        // Step 1: Create Account & Initial Lead
        if (!createdUserId) {
          ...
          if (authError) {
            ...
            // In a real app we'd handle login/recovery, but here we'll try to continue 
            // if we can identify the user or just alert.
        ...
        await supabase.from('onboarding_leads').update({
          ...
          goal: formData.goal,
          target_pain_point: formData.targetPainPoint,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const mpUrlWithRef = `${MP_SUBSCRIPTION_URL}&external_reference=${createdUserId}`;
      window.location.href = mpUrlWithRef;
  ...
  // We append external_reference so we can track this user in the Thank You page
  const mpUrlWithRef = `${MP_SUBSCRIPTION_URL}&external_reference=${newUserId}`;
  window.location.href = mpUrlWithRef;
          ...
          <AnimatePresence mode="wait">
            {currentStep === 1 && (
              ...
              >
                ...
                >
                  {isSaving ? <Loader2 className="animate-spin" size={20} /> : 'Siguiente Paso'}
                    ...
                    {VIDEO_FORMATS.map((format) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-3 rounded-full text-xs font-bold border-2 transition-all active:scale-90 ${isSelected ? 'bg-zylo-black text-white border-zylo-black' : 'bg-white text-gray-400 border-gray-100 hover:border-gray-200 shadow-sm'
                            }`}
                        >
              ...
              >
                ...
                <div className="flex flex-col gap-4 pt-6">
                  ...
                  <div className="flex gap-4">
                    <button onClick={prevStep} className="px-6 py-4 rounded-full border-2 border-gray-100 font-bold text-gray-400 hover:bg-gray-50 active:scale-95">Atrás</button>
                    <button
                      disabled={formData.competitors.length < 2 || isSaving}
                      onClick={handleNextStep}
                    ...
                    >
                      {isSaving ? <Loader2 className="animate-spin" size={20} /> : 'Revisar y Pagar'}
                      {!isSaving && <ChevronRight size={20} />}
                    </button>
                  </div>
                </div>
